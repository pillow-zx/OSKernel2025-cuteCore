...
impl LAFlexPageTableEntry {
    ...
    pub fn flags(&self) -> LAPTEFlagBits {
        LAPTEFlagBits::from_bits_truncate(self.bits & Self::PPN_MASK)
    }
    #[inline(always)]
...
impl PageTable for LAFlexPageTable {
    ...
    fn find_pte(&self, vpn: VirtPageNum) -> Option<&mut LAFlexPageTableEntry> {
    ...
    fn unmap(&mut self, vpn: VirtPageNum) {
        //tlb_invalidate();
        let pte = self.find_pte(vpn).unwrap(); // was `self.find_creat_pte(vpn).unwrap()`;
